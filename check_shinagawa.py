...
def parse_args():
    ...
    parser.add_argument("--weekdays", default="6,7,9")
...
def open_reservation_agreement(
    ...
    soup2 = BeautifulSoup(confirm_html, "html.parser")

    rsv_payload = {}


    for tag in soup2.find_all("input"):
    ...
    # ✅ 必要字段覆盖/补值
    rsv_payload.update({
        "applyNum": args.apply_num,

        "stimeZoneNo": str(raw.get("tZoneNo", "")),
        "etimeZoneNo": str(raw.get("tZoneNo", "")),

        "ppsCd": raw.get("ppsCd", ""),
        "ppsdCd": raw.get("ppsClsCd", ""),
    ...
    trace(f"rsv_payload_keys={list(rsv_payload.keys())}")
