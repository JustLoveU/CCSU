		...
		<Row :gutter="16">
			...
			<div>
				...
				<div style="margin: 10px 0;">
					...
					<Icon type="md-link" class="padding-left" /> <a :href="info.url"> {{info.source}}</a>
				</div>
				<div style="margin: 30px 5px;ont-size: 18px;
    letter-spacing: 1px;
    line-height: 32px;">
					<mavon-editor class="md" :value="info.content" :subfield="false" :defaultOpen="'preview'" :toolbarsFlag="false"
					 :editable="false" :scrollStyle="true" :ishljs="true" :boxShadow="false" previewBackground="#FFFFFF" />
				</div>
			</div>
			</Col>
			<Col span="8">
		...
		<div v-if="InfoList1.length != 0">
			...
			<List item-layout="vertical">
				<Row :gutter="16">
					...
					<div class="card">
						<a :href="'/info_detail?id='+item.id">
							<div :style="'width: 100%; height:200px; display: inline-block;background-image: url('+item.pic+');background-size: 100% 100%;border-radius:6px 6px 0 0;'">
...
<script>
	...
	export default {
		...
		methods: {
			getInfo() {
				this.$api.getRequest(this.$url.INTERFACES.infoOne + this.id).then(res => {
					if (res.status == 200) {
						this.info = res.info
