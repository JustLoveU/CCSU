...
public interface FriendDao {
    ...
    @Insert("INSERT INTO `lab_friend`( `url`, `name`, `status`, `pic`) VALUES ( #{url} , #{name} , #{status} ,#{pic} );")
    void save(FriendEntity friend);

    @Delete("delete from lab_friend where id = #{id} ")
    void removeByIds(Integer id);

    @Update("UPDATE `lab_friend` SET `url` = #{url}, `name` = #{name}, `status` = #{status}, `pic` = #{pic} WHERE `id` = #{id};")
