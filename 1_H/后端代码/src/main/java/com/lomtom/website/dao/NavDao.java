...
public interface NavDao{
    ...
    @Select("select b.* from nav a left join nav b on a.parent_id = b.id where a.id= #{id}  limit 1")
    NavEntity getFather(Integer id);


    @Select("select * from nav where parent_id = #{id}")
    List<NavEntity> getSon(Integer id);
